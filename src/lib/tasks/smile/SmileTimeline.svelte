<script lang="ts">
  ...
  export interface SmileTimelineMenuItem {
    label: string;
  ...
  interface Props {
    ...
    recordingType: ExpressionType | null;
    menuItems: SmileTimelineMenuItem[];
  ...
  function eventColor(type: ExpressionType, intensity: number, hovered: boolean): string {
    const hex = EXPRESSION_COLORS[type];
    const r = parseInt(hex.slice(1, 3), 16);
    const g = parseInt(hex.slice(3, 5), 16);
    const b = parseInt(hex.slice(5, 7), 16);
    const baseAlpha = 0.25 + intensity * 0.15;
    const alpha = hovered ? Math.min(baseAlpha + 0.3, 1) : baseAlpha;
  ...
  function hitTestEvent(x: number, y: number): SmileEvent | null {
  ...
  $effect(() => {
    ...
    for (const event of _events) {
      ...
      ctx.fillStyle = eventColor(event.type, event.intensity, event.id === _hoveredId);
    ...
    if (_recordingStart !== null && _recordingType !== null) {
      ...
      const barWidth = Math.max(x2 - x1, 2);
      const hex = EXPRESSION_COLORS[_recordingType];
      const r = parseInt(hex.slice(1, 3), 16);
      const g = parseInt(hex.slice(3, 5), 16);
      const b = parseInt(hex.slice(5, 7), 16);
